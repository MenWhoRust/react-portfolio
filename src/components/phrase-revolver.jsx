...
class PhraseRevolver extends React.Component {
  ...
  getPhrase(text) {
    ...
    return <div className={`revolver__phrase theme-dark ${this.state.phraseAnimClass}`}>{text}</div>;
  }
    ...
    return this.segmentArr
      .map((s, i) => {
        if (!phrasesReg.test(s)) {
          return (
            <div key={i} className='revolver__words theme-dark'>
              {s}
            </div>
          );
        }
        return (
          <div key={i} className='revolver__words theme-dark revolver__words--highlight'>
            {this.getPhrase(this.phrases[this.state.currentIndex])}
          </div>
        );
      })
      .reduce((a, b) => [a, " ", b]);
  ...
  render() {
    return <div className='revolver__container theme-dark'>{this.getRevolverContent()}</div>;
  }
